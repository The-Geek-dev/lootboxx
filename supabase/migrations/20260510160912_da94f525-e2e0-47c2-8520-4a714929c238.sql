...
BEGIN
  ...
  -- Only block while a previous request is still pending admin review.
  -- Approved/completed/rejected requests should not block new ones.
  SELECT count(*) INTO v_pending_count
  FROM public.withdrawals
  WHERE user_id = caller_id
    AND status = 'pending';
  IF v_pending_count > 0 THEN
    RAISE EXCEPTION 'You already have a withdrawal pending admin review. Please wait until it is approved or rejected before submitting another.';
  END IF;
